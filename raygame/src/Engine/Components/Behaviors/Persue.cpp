...
void Persue::update(float deltaTime)
{
	if (m_enabled)
	{
		if (m_owner && m_target)
		{
			m_owner->addForce(SteeringForce(m_target->getTransform()->getLocalPosition() + m_target->getVelocity(), m_owner->getTransform()->getLocalPosition(), m_maxSpeed) * m_weight * deltaTime);
