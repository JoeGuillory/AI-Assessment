...
namespace Pathfinding
{
	...
	bool NodeSortFScore(Node* i, Node* j)
	{
	...
	std::vector<Node*> AStar(Node* startNode, Node* endNode)
	{
		...
		while (!openList.empty())
		{
			std::sort(openList.begin(), openList.end(), NodeSortFScore);
			//Set the current node to the first node in the openList
			Node* currentNode = openList.front();
			//Remove currentNode from openList
			openList.erase(openList.begin());
			...
			for (Edge e : currentNode->connections)
			{
				...
				if (std::find(closedList.begin(), closedList.end(), e.target) != closedList.end()) 
				{
					currentNode->gScore = currentNode->gScore + e.cost;
					currentNode->hScore = Heuristic(e.target, endNode);
	...
	float Heuristic(Node* target, Node* endnode)
	{
		...
		return abs(target->position.x - endnode->position.x) + abs(target->position.y - endnode->position.y);
	}
